namespace Ais.Office.Areas.OutAdministrativeAct.Controllers
{
    ...
    /// <summary>
    /// Class AdmActPublishedInTermController.
    /// Implements the <see cref="Ais.Table.Mvc.Controllers.SearchTableController{AdmActPublishedInTermReportsQueryViewModel, AdmActPublishedInTermReportsTableViewModel}" />
    /// </summary>
    /// <seealso cref="Ais.Table.Mvc.Controllers.SearchTableController{AdmActPublishedInTermReportsQueryViewModel, AdmActPublishedInTermReportsTableViewModel}" />
    [Area("OutAdministrativeAct")]
    [Authorize(Roles = UserRolesConstants.AAPublishedInTermReportsSearch)]
    public class AdmActPublishedInTermReportsController : SearchTableController<AdmActPublishedInTermReportsQueryViewModel, AdmActPublishedInTermReportsTableViewModel>
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="AdmActPublishedInTermReportsController"/> class.
        /// </summary>
            ...
            : base(logger, localizer, sessionSessionStorageService)
        {
            ...
            this.Options.TableHeaderText = this.Localizer["AAPublishedInTermReportsSearch"];
            this.addressService = addressService;
            this.nomenclatureService = nomenclatureService;
        }

        public override Task<IActionResult> Index(AdmActPublishedInTermReportsQueryViewModel query = null)
        {
            if (query == null || !ReflectionUtils.HasNonNullProperty(query))
            {
                query = new AdmActPublishedInTermReportsQueryViewModel();
            }
        ...
        protected override async Task InitialQueryAsync(AdmActPublishedInTermReportsQueryViewModel model)
        {
        ...
        protected override async Task<IEnumerable<AdmActPublishedInTermReportsTableViewModel>> FindResultsAsync(AdmActPublishedInTermReportsQueryViewModel query)
        {
            var dbQuery = this.mapper.Map<AdmActPublishedInTermReportsQueryModel>(query);
            List<AdmActPublishedInTermReportsTableModel> results;
            await using (await this.contextManager.NewConnectionAsync())
            {
                results = await this.outAdmActService.SearchAdmActPublishedInTermReportsAsync(dbQuery);
            }

            return this.mapper.Map<IEnumerable<AdmActPublishedInTermReportsTableViewModel>>(results);
        }
    }
}
